...
def swing(provider='[default]', port=9876):
	...
	if system.tag.exists('%s%s' % (provider, crowbar_config['name'])):
		# if it's already there, kill it
		system.tag.writeBlocking(['%s%s' % (provider, crowbar_config['name'])], [False])
	else:
		system.tag.configure(provider, crowbar_config)
	
	sleep(0.5)
	
	system.tag.writeBlocking(['%s%s' % (provider, crowbar_config['name'])], [True])

# This is, admittedly, a bit silly.
# But by wrapping it all here we can make sure that `swing` is an option even from the designer.
# And that's nice.
try:
	try:
	...
	pass # we're not in a gateway context here, so we can't load in crowbar.
